...
void audio_ace::task()
{
    ...
    while (!quit_) {
        ...
        if (!out_buff->data()) 
            out_buff->alloc(INS_AUDIO_FRAME_SIZE*sizeof(short));
        ...
        speex_echo_cancellation(st_, 
                                (spx_int16_t*)(mic_frame.buff->data()+mic_frame.buff->size()/2), 
