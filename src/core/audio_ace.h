...
public:
    ...
    void queue_mic_frame(const ins_pcm_frame& frame)
    {
        queue_frame(frame, AEC_QUEUE::MIC);
    }
    void queue_spk_frame(const ins_pcm_frame& frame)
    {
        queue_frame(frame, AEC_QUEUE::SPK);
    }
    // bool dequeue_aec_frame(ins_pcm_frame& frame)
    // {
    //     return dequeue_frame(frame, AEC_QUEUE::AEC);
    // }
    void set_frame_cb(std::function<void(ins_pcm_frame& frame)> cb)
    {
        frame_cb_ = cb;
    }
private:
