...
int32_t audio_mgr::open_outer_usb_mic(bool hdmi_audio)
{
    ...
    if (fmt_size_ == 3) buff_size_ = 1024*2*sizeof(short);
...
void audio_mgr::on_pcm_data(uint32_t index, ins_pcm_frame& frame)
{
    if (dev_type_ == INS_SND_TYPE_USB)
    {
        ...
        { 
            ...
            if (!buff_s16->data()) buff_s16->alloc(frame.buff->size()/3*2);
        ...
        enc_[0]->encode(frame);
        if (play_) play_->queue_frame(frame);
    }
...
int32_t audio_mgr::align_first_frame()
{
    ...
    {
        //wait dev ready
        if (dev_[0]->base_time_ != -1 && dev_[1]->base_time_ != -1) break;
        usleep(10*1000);
        if (quit_) return INS_ERR;
    }
    ...
    while (1)
    {
        ...
        if (frame_0.pts == -1) deque_pcm_frame(0, frame_0);
        if (frame_1.pts == -1) deque_pcm_frame(1, frame_1);

        if (frame_0.pts == -1 || frame_1.pts == -1) continue;
...
void audio_mgr::process(std::vector<ins_pcm_frame>& v_frame)
{
    ...
    if (enc_.size() > 1 || play_) 
    {
        ...
        if (enc_.size() > 1) enc_[1]->encode(v_frame[0]);
        if (play_) play_->queue_frame(v_frame[0]);
    }

    auto buff_4ch = pool_->pop();
    if (!buff_4ch->data()) buff_4ch->alloc(v_frame[0].buff->size()*2);
...
void audio_mgr::queue_pcm_frame(uint32_t index, ins_pcm_frame& frame)
{
    ...
    if (queue_[index].size() > 200)
    {
        ...
        for (int32_t i = 0; i < 10; i++) queue_[index].pop_front();
    }
...
{
    std::lock_guard<std::mutex> lock(mtx_[index]);
    if (queue_[index].size() < 2) return false;
