...
void storage_speed_test::produce_task(int index)
{
    ...
    double total_size = ((double)size*(loop_cnt-queue_[index].size()))*8.0/1024.0/1024.0;

    struct timeval end_time;
    gettimeofday(&end_time, nullptr);
    long long total_time = end_time.tv_sec*1000*1000 + end_time.tv_usec - start_time.tv_sec*1000*1000 - start_time.tv_usec;

    double speed = total_size*1000*1000/total_time;

    LOGINFO("index:%d total size:%lf mb speed:%lf", index, total_size, speed);
