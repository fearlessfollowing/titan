...
private:
    ...
    std::vector<std::shared_ptr<nv_video_dec>> 			dec_;		/* 解码器容器 */
    std::map<uint32_t, std::shared_ptr<nv_video_enc>> 	enc_;		/* 编码器map ,每个编码器对应一个index */
    ...
    std::thread 						th_dec_[INS_CAM_NUM];		/* 解码线程（一个模组对应一个解码线程） */
    std::thread 						th_enc_;
    std::mutex 							mtx_;
