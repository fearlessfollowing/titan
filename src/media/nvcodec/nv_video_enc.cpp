...
int32_t nv_video_enc::open(std::string name, std::string mime)
{
    ...
    LOGINFO("%s open mime:%s w:%u h:%u bitrate:%u fps:%d/%d", 
		name_.c_str(), mime.c_str(), width_, height_, bitrate_, framerate_.num, framerate_.den);
...
bool nv_video_enc::capture_plane_done(struct v4l2_buffer *v4l2_buf, NvBuffer *buffer, NvBuffer *shared_buffer)
{
	...
	/* 重新将该NvBuffer丢入到capture_plane的队列中 */
    if (enc_->capture_plane.qBuffer(*v4l2_buf, nullptr)) {
		LOGERR("--> Error while Qing buffer at capture plane");
        NV_ENC_ABORT();
        return false; 
    }
...
int32_t nv_video_enc::queue_intput_buff(NvBuffer* buff, int64_t pts)
{
	...
	int ret = -1, i = 0;
	do {
		ret = enc_->output_plane.qBuffer(v4l2_buf_, nullptr);
		if (ret < 0) { 
			LOGERR("%s output plane qbuffer fail:%d error:%d %s", name_.c_str(), ret, errno, strerror(errno));
			usleep(10*1000);
		} else {
	...
	if (ret) {
		LOGINFO("-----cpu:%d GPU:%d", hw_util::get_temp("sys.cpu_temp"), hw_util::get_temp("sys.gpu_temp"));
        system("cat /proc/meminfo >> /home/nvidia/mem.txt");
		NV_ENC_ABORT();
        return INS_ERR;
	}
