...
public:
	...
	virtual void 	set_event_cb(std::function<void(int)> event_cb) { event_cb_ = event_cb; }
	virtual void 	set_mux_io_option(std::map<std::string, std::string>& option) { mux_io_opt_ = option; }
	virtual void 	flush() { b_flush_ = true; }
