...
public:
	...
	void set_gps(bool value) {
		b_gps_ = value;
		if (b_gps_) 
			gps_buff_ = std::make_shared<insbuff>(camm_util::gps_size);
	}
...
private:
	...
	int32_t write_gyro_frame(amba_gyro_data* gyro) {
		/** 1.将Amba的原始陀螺仪数据存储到陀螺仪的insbuff中 */
		#ifdef GYRO_EXT
	...
	std::queue<std::shared_ptr<ins_frame>> 			video_queue_;		/* 视频数据队列 */
	std::queue<std::shared_ptr<ins_frame>> 			audio_queue_;		/* 音频数据队列 */
	std::queue<std::shared_ptr<ins_gps_data>> 		gps_queue_;			/* 存储GPS数据的队列 */
	std::queue<std::shared_ptr<insbuff>> 			gyro_queue_;		/* 存储陀螺仪数据的队列 */
	std::queue<std::shared_ptr<exposure_times>> 	exp_queue_;			/* 存储曝光时间数据的队列 */
	std::shared_ptr<ins_frame> 						last_key_frame_;	/* 最后一个关键帧 */

	bool 						b_auto_connect_ = false;				/* 是否为自动连接(Live) */
	int 						auto_connect_interval_ = 0;				/* 自动重连的间隔 */
	int 						auto_connect_cnt_ = 0;					/* 自动重连的次数 */
	int 						retry_cnt_ = 0;
