...
protected:
	...
	void prepare_vao();
	void draw_master_vao(int w, int h, const float* mat, const std::vector<const unsigned char*> image = std::vector<const unsigned char*>()) const;
	void draw_aux_vao(GLuint texture) const;

private:
	static const int vao_num_ = 1;
	static const int vbo_num_ = 2;
	static const int vao_aux_num_ = 1;
	static const int vbo_aux_num_ = 2;
	
	void release_render_cicle();
	void prepare_master_vao(GLuint program, int width, int height, float* vetex_data, GLuint* index_data);
	...
	void active_img_texture(const std::vector<const unsigned char*>& image) const;
